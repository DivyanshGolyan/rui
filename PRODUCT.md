...
The Run interface demonstrates durable workflow and Session identity, keyed reattachment, exact Action authority, crash recovery, explicit uncertainty, patch reconciliation, executable verification, and honest resource accounting. Deterministic fixtures provide reproducible fan-out, replay, repair, and targeted crash demonstrations. Codex is the required first live provider target through one model-only transport using the user's existing ChatGPT subscription. The first live proof deliberately precedes workflow orchestration: a real Codex model must inspect and repair a controlled repository through the existing capacity-one Harness, canonical Conversation, and admitted Bash and patch tools. The same adapter is later carried unchanged into durable Workflow Runs and measured concurrency. OnePage may claim the subscription experience only after the official authorization and transport feasibility gate passes; it never embeds or delegates to a second agent loop.
