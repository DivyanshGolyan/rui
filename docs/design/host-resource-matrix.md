...
[Host memory accounting](host-memory-accounting.md) combines current measured costs and named allocations without adding whole-process baselines twice or inventing component allowances. The 256 MiB target remains unqualified: Bash supervision, final shared state, evaluator population/containment and full mixed-owner overlap still need concrete derivation and integrated evidence.
