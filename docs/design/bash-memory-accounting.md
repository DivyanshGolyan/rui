...
- [Execution](../../src/bash_tool.zig#L51) owns allocated stdout/stderr slices and frees them on deinit.
- [runBounded](../../src/bash_tool.zig#L259) spawns Bash with two pipes, passes an allocator into MultiReader, and accumulates buffered output until EOF, cancellation, timeout or the historical output threshold. It does not drain captured bytes into scratch as they arrive.
...
- [Lifecycle admission](../../src/lifecycle.zig#L1049) calls the executor, then stores the returned result while its captured slices remain live. This is not the selected direct scratch-evidence interface.
