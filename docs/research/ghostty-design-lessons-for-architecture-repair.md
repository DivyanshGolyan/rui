...
OnePage currently keeps 29 raw Wasm function references, numeric state interpretation, JavaScriptCore mechanics, and a mutating compatibility probe inside `agent.zig` ([`src/agent.zig:68-240`](../../src/agent.zig)). The one-page policy core itself exposes many scalar getters ([`src/core.zig:80-323`](../../src/core.zig)). Moving this code unchanged into `core_native.zig` would improve locality only cosmetically; the module would remain shallow because every Harness caller would still understand raw export names, offsets, and numeric states.
...
The current `Core.open` compatibility probe drives a full sequence of mutations on the instance that will later be used ([`src/agent.zig:158-240`](../../src/agent.zig)). Replace it with structural artifact verification plus a real-artifact integration test, or run behavioral conformance on a disposable instance. Opening a production Core must leave it in one documented initial state.
...
OnePage's current generic `Transition` makes `classify`, `persist`, and `apply` independently fallible ([`src/harness.zig:66-71`](../../src/harness.zig)). That permits a durable fact to commit and then fail while applying its corresponding Core state. Marking the owner failed is necessary but does not make the transition contract clear or mechanically safe.
