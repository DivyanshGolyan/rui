...
OnePage already has the most important prerequisite: it distinguishes durable Core State and immutable blobs from an exact 64 KiB resident Activation Slot. Its normative lifecycle says that `drive` borrows a slot for one owner quantum, commits and encodes state, scrubs the slot, and releases it before returning ([ARCHITECTURE.md](../../ARCHITECTURE.md)). That is the OnePage equivalent of preserving Ghostty's terminal state while releasing the renderer's cache.
...
1. **Make coldness an internal Harness/Host-Runtime lifecycle state, not a new public lifecycle.** Keep `Harness.open / offer / drive` as the only Session seam. Internally, permit a Harness to retain only identity/cursors while it releases decoded Session indexes and any activation cache after a committed suspension. Reopening that warm state must follow the same bounded recovery path as crash recovery. This avoids a large number of open-but-waiting Harness objects becoming an accidental resident agent population.
