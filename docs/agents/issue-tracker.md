...
- **Read an issue**: `gh issue view <number> --json number,title,body,state,labels,comments --jq '{number, title, body, state, labels: [.labels[].name], comments: [.comments[] | {author: .author.login, createdAt, body, url}]}'`.
...
When a skill says to publish to the issue tracker, create a GitHub issue. When it says to fetch a ticket, use the **Read an issue** command above.
