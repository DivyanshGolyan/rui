...
contract and [qualification targets](../../VERIFICATION.md#accepted-host-qualification-targets)
...
This amends earlier statements excluding temporary-file reclamation or assuming all tool output is removed at settlement. Spillover loss on crash, Host exit or reclamation is accepted; saved results remain unchanged, and recovery never automatically reruns an old Bash call. Retained-file metadata and access still require bounded ownership. The [current retention contract](../../ARCHITECTURE.md#shared-temporary-file-retention) and [memory verification](../../VERIFICATION.md#memory-and-density) own the requirements. Historical measurements do not certify the added retained population; no production implementation is claimed.
