...
OnePage's memory guarantee covers resources owned by the harness: Activation Slots, live Harness state, Host Runtime metadata, SQLite, provider transport, bounded adapter capture, and the one live caller-side workflow evaluation. Their resident working set must be a function of explicit host capacities and current in-flight work, not total durable Sessions, Conversation length, historical opens, or waiting Workflow Runs. Terminal Jobs add durable bytes, not resident per-Job objects; a live evaluation may scale only to its bounded current Visibility Snapshot under its fixed Workflow Resource Profile and retains no cumulative state from earlier evaluations.
...
- a Dormant or closed Session and a waiting Workflow Run retain no live Harness allocation, Active Credit, thread, socket, subprocess, JavaScript heap, or Promise graph; an In-flight Session retains only its one credit and bounded admitted-adapter resources, never a Harness or Slot;
...
- transport, SQLite, workflow evaluation, runtime stacks, allocator overhead, kernel socket memory, and subprocesses are measured as separate categories rather than hidden inside the Activation Slot claim.
