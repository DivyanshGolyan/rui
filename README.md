...
JavaScript workflows, Edit and structured answers are not implemented. Rui-owned Codex login, private credentials and managed transport have deterministic/native fixture coverage. At `3d4c0e0`, an augmented `gpt-6-luna` journey passed natively on Linux and macOS: private reasoning in an earlier request, then exact approved Bash, continuation and fresh-Host recovery. The first Bash proposal itself contained no private reasoning, so the precise [#272 proof diagram](https://github.com/DivyanshGolyan/rui/issues/272) remains unqualified; live refresh was not exercised. Bash process-group stopping cannot contain detached descendants. Retained full output is optional: FIFO eviction or restart may remove it without changing the saved result. Power-loss and complete 1,000-operation mixed qualification remain outstanding.
...
An authorized native qualification can run `python3 tests/integration/codex_live.py /absolute/path/to/rui --live --model EXACT_MODEL` separately on Linux and macOS. It prompts for Rui-owned device login, establishes real private reasoning in the same Session before enabling Bash, approves only its exact inspected harmless Bash proposal and reports payload-free observations; it is not in ordinary build gates. The augmented journey passed on both platforms, but the issue's first-response pairing is still unproved, so this is not yet a qualified live-provider quickstart.
...
The runnable #260 slice and implemented [Codex path](https://github.com/DivyanshGolyan/rui/issues/272) are described above. The narrower first-response live proof still gates that issue's closure. A [minimal durable workflow](https://github.com/DivyanshGolyan/rui/issues/273) remains planned; these links do not expand the implemented surface described in [Status](#status).
