...
`zig build codex-credential-integration` also tests cross-process credential mutation without live tokens. An authorized native qualification can run `python3 tests/integration/codex_live.py /absolute/path/to/rui --live --model EXACT_MODEL` separately on Linux and macOS. It prompts for Rui-owned device login, establishes real private reasoning in the same Session before enabling Bash, approves only its exact inspected harmless Bash proposal and reports payload-free observations; it is not in ordinary build gates. The accepted journey passed on both platforms, but remaining resource evidence prevents a qualified live-provider quickstart.
