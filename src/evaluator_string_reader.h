 ...
 * This is a private extension tied to the pinned QuickJS source. */
...
/* Borrow chunks of the pinned engine string's code units for the duration of
 * each callback. wide selects uint16_t rather than uint8_t units. The callback
 * must not invoke JS or retain units. No string-sized native copy is made. */
typedef int (*OPStringUnits)(void *opaque, const void *units,
                             size_t length, int wide);
int OP_VisitStringUnits(JSValueConst value, OPStringUnits visit, void *opaque);
#endif
