...
test "runtime Slot pool owns one fixed allocation and fences stale releases" {
    var pool = try RuntimeSlotPool.init(std.testing.allocator, 2);
    defer pool.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(usize, 2), pool.capacity());
    try std.testing.expectEqual(2 * @sizeOf(ActivationSlot), pool.residentBytes());

    var first = try pool.borrow();
    var stale = first;
    var second = try pool.borrow();
    try std.testing.expectError(error.ActivationCapacityExhausted, pool.borrow());
    try std.testing.expectEqual(2 * @sizeOf(ActivationSlot), pool.occupiedHighWaterBytes());
    try first.release();

    var reused = try pool.borrow();
    reused.slot.state.agent_id = 99;
    try std.testing.expectError(error.StaleSlotLease, stale.release());
    try std.testing.expectEqual(@as(u64, 99), reused.slot.state.agent_id);
    try reused.release();
    try second.release();
}
...
test "runtime Slot pool admits exactly its capacity under concurrent borrowing" {
    ...
    const high_water_is_exact =
        pool.occupiedHighWaterBytes() == 4 * @sizeOf(ActivationSlot);
    ...
    try std.testing.expectEqual(@as(usize, 0), pool.occupiedBytes());
}
