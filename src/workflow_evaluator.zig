...
const probe_c = @cImport({ @cInclude("stdio.h"); });
...
const Evaluation = struct {
    ...
    fn parse(allocator: std.mem.Allocator, input: []const u8) !*Evaluation {
        ...
        comptime { if (protocol.Limits.data_entries * @sizeOf([]const u8) < protocol.Limits.workflow_output_bytes) @compileError("probe reuse capacity insufficient"); }
        ...
        state.* = .{
            ...
            .source = source,
            .arguments = arguments,
            .descriptor_scratch = std.mem.sliceAsBytes(key_storage)[0..protocol.Limits.workflow_output_bytes],
