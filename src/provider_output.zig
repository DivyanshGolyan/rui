...
pub const ReplayCursor = struct {
    ...
    pub fn advance(
        ...
        while (bytes_left.* != 0 and items_left.* != 0) switch (self.phase) {
            ...
            .object_start => {
                const byte = try self.take(reader, bytes_left) orelse return error.UnexpectedJsonEnd;
                if (isSpace(byte)) continue;
                if (byte != '{') return error.UnexpectedJsonDelimiter;
                self.phase = .field_start;
            },
            .field_start => {
                const byte = try self.peek(reader, bytes_left) orelse return error.UnexpectedJsonEnd;
                if (isSpace(byte)) {
                    _ = try self.take(reader, bytes_left);
                    continue;
                }
                if (byte == '}') {
                    _ = try self.take(reader, bytes_left);
            ...
            .copy_field => {
                ...
                const wanted: usize = @intCast(@min(remaining, @min(bytes_left.*, self.buffer.len)));
                const count = try reader.read(self.copy_position, self.buffer[0..wanted]);
            ...
            .source_end => {
                const byte = try self.peek(reader, bytes_left) orelse {
                    self.phase = .close;
                    continue;
                };
    ...
    fn peek(self: *ReplayCursor, reader: *store.HistoricalReader, bytes_left: *usize) !?u8 {
        if (self.position >= reader.reference.length) return null;
        if (self.position < self.buffer_start or self.position >= self.buffer_start + self.buffer_length) {
            if (bytes_left.* == 0) return null;
            self.buffer_start = self.position;
            const wanted: usize = @intCast(@min(reader.reference.length - self.position, @min(self.buffer.len, bytes_left.*)));
            const count = try reader.read(self.position, self.buffer[0..wanted]);
            if (count != wanted) return error.ShortCanonicalRead;
            self.buffer_length = count;
        }
        return self.buffer[@intCast(self.position - self.buffer_start)];
    ...
    fn take(self: *ReplayCursor, reader: *store.HistoricalReader, bytes_left: *usize) !?u8 {
        if (bytes_left.* == 0) return null;
        const byte = try self.peek(reader, bytes_left) orelse return null;
        self.position += 1;
        bytes_left.* -= 1;
        return byte;
    }
