...
| Session observation has one bounded value | Lifecycle consumers read one immutable `SemanticView` containing the current model and consequential Operations, relevant Attempts, control state, indeterminate Result, ledger head, and committed Core State. Production has no caller-supplied replay callbacks or parallel kind/recovery indexes. |
| Operation meaning has one encoding | Codec and lifecycle tests prove one `operation_admitted` fact binds each opaque Operation identity, the descriptor tag is the sole effect-kind classifier, an Action explicitly names its source model Operation, and no identity bits or parallel recovery field carry those meanings. |
| Session owns Core publication | Core-bearing commit tests pass the live Core to Session, prove Session rejects a mismatched identity, and prove callers cannot supply an independently encoded Core State beside semantic facts. Activation restores only the latest Core State held by the committed Semantic View. |
