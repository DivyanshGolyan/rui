...
| Operation meaning has one encoding | Codec and lifecycle tests prove one `operation_admitted` fact binds each opaque Operation identity, the descriptor tag is the sole effect-kind classifier, and an Action explicitly names its source model Operation's complete `(ID, generation)` identity. Attempts exactly match that descriptor binding; Authorization, Approval Required, Result application, Completion recovery, identity bits, and parentage carry no parallel effect-kind classifier. Bash/patch mismatch and reused-ID generation regressions fail closed. |
| Session owns Core publication | Core-bearing commit tests pass the live Core to Session, prove Session rejects a mismatched identity, and prove the reducer's typed pending change must exactly match the fact shape and values. Callers cannot supply unrelated semantic facts or an independently encoded Core State beside the reduction. Activation restores only the latest Core State held by the committed Semantic View. |
