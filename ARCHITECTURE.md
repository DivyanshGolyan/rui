...
**Authorization** durably permits one exact validated Action. At applicable-Action admission select current Session Permission Mode, default `bypass`, and save descriptor plus configuration provenance. Rejected Tool Calls never consult permission policy.

`ask` creates one immutable Permission Request; `bypass` directly creates Authorization. Siblings admitted together share that view. Permission Decisions allow once or deny one exact request/Operation/descriptor under Local Owner authority; server access never implies bypass.
...
The CLI has two explicit modes. `rui session --store PATH --session REF` enters an already configured Session on a terminal, displays its effective Workspace and permission mode (including a bypass warning), and retains that address only within the interactive process. Plain input submits a new Message, captures its exact request before transmission, and follows that message's own key to an answer or actionable permission. The CLI receives terminal lines before canonical-mode truncation, rejects overlong or unsupported control input before capture, discards typeahead from the previous prompt when beginning each new prompt, and restores terminal settings before releasing input to the Host on normal exit and Ctrl+C at a prompt; longer content uses the one-shot file route. Typeahead entered before an Action prompt cannot authorize that Action. On attention it presents the exact Action ID and complete arguments with terminal-safe escaping, preserving stored bytes, then offers allow-once, deny or later; the opaque provider call ID remains available through one-shot inspection. An affirmative choice remains a separate durable exact-target command, revalidated by the Host. Successful interactive submissions and decisions omit protocol receipts while retaining their saved local recovery handles; uncertain mutation failures direct the caller to `/requests`. Completed answers appear without duplicate outcome labels. `/configure` changes settings for this Session (including its immutable Workspace check), but file-valued settings must use a path rather than reading terminal stdin; one-shot configuration still accepts `-`. `/status` inspects current and recent Host facts, `/wait` selects current work once, `/requests` lists local recovery handles for this destination, and `/exit` detaches. Host-returned identities and other human fields are escaped for terminal display, without changing their exact bytes for selection or JSON. Re-entering uses the Host's facts, not local saved-request guesses. An unknown Session requires explicit configuration outside the interactive loop. Terminal input, stdout and prompting are required; redirection rejects rather than silently changing semantics. Exit, interruption or a disconnected caller never cancels Host work.

**Bounded CLI editor (native terminal-display qualification pending):** The interactive terminal reader owns the draft, grapheme cursor, keyboard/paste decoding, terminal rendering, input rejection and terminal lifetime for each prompt. Its caller supplies storage: up to 65,536 UTF-8 bytes for a Message, and a smaller choice buffer. One active prompt returns a borrowed complete line only after successful validation; it retains no read-ahead, history or approval across prompts. Strict UTF-8 and stateful extended-grapheme segmentation govern edits, including when deletion joins previously separate clusters; byte count governs the limit, and uncertain terminal cell widths cannot justify cursor movement. Ctrl-D exits only an empty draft; otherwise it deletes the following grapheme or does nothing at the end. Normal editing operates on the current logical line, including marked multiline paste as one Message; ordinary Enter submits. An over-limit, malformed or unsafe draft is rejected in its entirety even if later edited shorter; an incomplete UTF-8 scalar or marked paste times out and ends the interactive caller after terminal restoration so a delayed tail cannot become input to a later prompt. If cursor placement cannot be established safely during an edit (including ambiguous width, wrap or resize), decline the edit with an explicit failure and no request rather than paint or submit a misleading draft; direct the caller to one-shot `message --text FILE`. Restore terminal settings and disable bracketed paste on all exits, reporting unconfirmed cleanup after an I/O failure rather than claiming restoration. The CLI still interprets accepted text or a fresh explicit `a`/`d`/`l` choice, and the Host alone validates the exact Action. Discard old input at each prompt boundary, including before presenting a fresh Action choice; terminal-marked paste cannot make that choice. A terminal may consume Option- or Command-Backspace before Rui sees it; only supported received key sequences are promised. This design adds no Host or Workflow input policy, history, completion, full-screen UI or configurable keymap.

An incomplete CSI or SS3 sequence also times out and ends the caller, rather than treating its delayed tail as Message text; bare ESC alone may expire without rejecting the draft.

Consecutive Backspaces at the draft's end may be applied with two forward grapheme scans and one repaint on a bounded cadence. Check the original draft's display geometry before applying the batch, and flush it before a different key or prompt exit; this changes presentation cadence, not the Message bytes or terminal/Host handoff. When the prompt and draft have only printable ASCII and have never wrapped or resized, each tail Backspace may erase one known-width cell directly, even when keys arrive separately. Separately paced Unicode edits can still require a full repaint each; the editor does not promise linear total work for arbitrary Unicode editing.
...
The human `result` first observes the saved message key and presents its original rejection, queued/processing state, exclusion, failure, cancellation or completion as data; it streams the public answer only when completed. It does not infer a work failure from a transport/observation error or substitute the Session's latest result. Before a Permission Decision, the interactive CLI shows the numeric Action ID and complete arguments without reading the hidden provider call ID; one-shot inspection also shows the call ID. A separate allow/deny names the exact Action target; neither presentation nor `follow` authorizes an effect.
