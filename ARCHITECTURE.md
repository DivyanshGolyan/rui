...
The human-facing CLI uses one-shot commands over those public operations, with no implicit current Session or persistent interactive client. Each direct-core mutation names its Store and full Session reference, plus the exact Action target when applicable; Workflow commands name their Workflow target. Action inspection escapes provider-controlled text before showing it to a person, without changing the stored bytes or the Host's exact-target validation. `follow` is an optional foreground observation that detaches without cancelling work. A direct-core mutation generates a fresh core key and durably captures its exact destination, target and inputs in one private, enumerable caller record before transmission. The record is its recovery handle: a fresh client can list saved requests, observe or retry that same captured request after an uncertain reply, and deliberately create a new record for new intent. If the process exits after capture but before printing the handle, enumeration still finds it. Records retain request-recovery custody only; they are not a local Session directory, job database, dispatch queue or source of work status. The existing explicit-key/record commands remain available to programs and for debugging. The Host starts explicitly.
...
Human `follow` selects the original saved message key, not whatever Turn is current later. It may return on that message's terminal outcome or an actionable Permission Request requiring attention, even while a sibling Action progresses. Its return reason is separate from the authoritative work status: report the progressing sibling as in flight, not the whole Session as blocked. A queued message behind a failed Turn does not inherit that Turn's failure. Closing `follow` changes no work or saved request. This attention condition does not alter the default Session-wait predicate or introduce a Session phase.
