...
Configuration and messages share one Store-wide core idempotency-key domain across all callers. Session reference, operation kind and complete canonical inputs are bound inputs, not lookup scope: changing any of them under an existing key conflicts. Atomically bind them and the first committed acceptance or definite rejection with the corresponding mutation. Recovery remains subject to authorization; a key grants no access to the saved answer.
