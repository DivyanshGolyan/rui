...
| Qualification, not admission | Whole Latifa footprint ≤256 MiB in the current Active Capacity 1/8/16/100 model-slice cases and the eventual 1,000-operation model/Bash/Edit/mixed finished-product fixtures, including helpers; same cold/retained-idle ceiling. Idle CPU <1% one core. Current model-slice qualification runs capacities 1/8/16 at 30 events/s per stream and 100 at 100 events/s per stream for 60 seconds; each case consumes ≤2 Host cores on average across a conservatively bracketed interval of at least 40 seconds wholly inside simultaneous offered work, and ≤120 Host CPU seconds from before admission through every-key result audit and observed cleanup. The default-capacity 1,000-operation proof remains part of the assembled finished-product qualification, not this slice. p95 durable control acknowledgment ≤1 s; light/free-capacity retry discovery ≤2 s after due. |

### Qualification measurements

Qualify stable memory across churn/history growth. The current model CPU fixture runs Active Capacity 1/8/16 at 30 events/s per stream and capacity 100 at 100 events/s per stream for 60 seconds, with short requests and bounded terminal bursts. Each SSE record is a realistic 260 bytes and each batch contains one event, so each ordinary stream offers exactly 1,800 events and each stress stream exactly 6,000. Release ordinal `i` at the exact rational offset `floor(i × 60 seconds / event count)`, avoiding truncated-interval drift, and complete every event exactly once. Completion must not precede that original rational release and must occur no later than two ceiling intervals after it. To prevent a delayed fixture wakeup from turning into a catch-up burst, also wait until the completion two ordinals earlier plus one ceiling interval; this lower bound never redefines the intended release. Adjacent completions may be at most two ceiling intervals apart and every half-open ceiling-interval window may contain at most two completions. Never skip late work. Stop only on transport/cancellation failure or the offer hard deadline two ceiling intervals after 60 seconds. Run two same-Host rounds where churn evidence is required. Defer the default-capacity 1,000-operation workload to assembled finished-product qualification.
