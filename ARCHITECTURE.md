...
Read in that order, or go directly to the owning contract:
...
Incomplete initialization and messages to unknown Session references reject without partial Sessions. Message admission checks all locally decidable continuation preconditions without provider I/O. Configuration enqueues no message; any previously queued input remains governed by the queue invariant when changed settings make it resumable.
...
These commands share one Store-wide core idempotency-key domain across all callers. Target reference, operation kind and complete canonical inputs are bound inputs, not lookup scope: changing any of them under an existing key conflicts. Atomically bind them and the first committed acceptance or definite rejection with the corresponding mutation. Recovery remains subject to authorization; a key grants no access to the saved answer.
...
Recover a matching existing admission answer before reevaluating current admission conditions; changed inputs conflict without replacement. A fresh intended submission needs a fresh identity, even for equal input. Correctness assumes callers allocate a distinct key for each new command and reuse it only for retries. V1 accepts the probabilistic uniqueness of securely generated UUIDv4 keys. Core cannot distinguish an independently intended command with the same key and inputs from a retry; parameter comparison detects mismatched reuse, not all identity collisions.
...
At an input boundary, select all eligible pending messages through the observed admission cutoff. Successful selection atomically binds that prefix to the processing Turn and appends its Conversation projections; for active work this commits with the next assistant-response model Operation. If local preparation rejects the selected prefix before projection, the terminal-failure transaction binds that prefix to the failing Turn and records the cause without inventing projections or a model Operation. Recheck selection eligibility, stop authority and the parent Turn at commit; a saved stop cannot be overwritten by late preparation. Rollback commits neither selection nor new projections. Input beyond the selected cutoff remains queued. Binding is established once: the message observes that Turn's final text, schema-validated value or typed failure, including failure before projection. It is never rebound or automatically requeued after processing. Stop/cancellation may instead exclude an unselected admission and provide its cancelled result through the controlling fact, without inventing a Turn.

**Queue invariant:** eligible queued input makes a resumable Session runnable, without requiring another message. Eligible input is admitted, unselected and not excluded by stop/cancellation. Resumability requires the ordinary continuation and effect-settlement preconditions; queue presence grants no bypass. Core discovers this work from committed facts after admission, input boundaries, terminal settlement, prerequisite changes and restart, even if a notification is lost. Under fair service, it advances when capacity and those preconditions permit; unavailable capacity retains work without execution resources or a spin loop. A prerequisite failure follows its existing typed-failure or shutdown boundary. Selecting input for a failed preparation settles that input once; it cannot repeatedly restart itself. Runnable state is derived, not a persisted Session phase, wake receipt or per-Session worker.

One eligibility rule covers arrival before or after failure and recovery, removing a special dependency on a later caller message. This is the invariant-based simplification described in [Matklad's discussion of invariants](https://matklad.github.io/2023/10/06/what-is-an-invariant.html); eventual scheduling is the accompanying progress guarantee.
...
At fresh assistant-response request admission, select all not-yet-included instruction updates through that transaction's configuration view. After preceding Tool Results and applicable User Message projections, append them in configuration order at the fixed provider-supported position.

Validate replay and commit entries, projections, Operation and manifest together. Inclusion derives from canonical originating-update references and initial baseline, including compaction-covered entries, not text equality or a separate queue/applied flag/receipt. Rollback leaves admitted updates pending. Already-admitted requests and retries retain frozen inputs; inclusion does not prove provider consumption.

Keep the initial instruction prefix stable rather than rendering prior positions from current settings. This specifies placement, not global wall-clock interleaving with messages.
...
The local owner orders completion/interruption, fences further result publication and suppresses an unconsumed permit or detaches active transport. Terminal evidence delivery is at most once; suppress duplicate/stale delivery before custody reuse, without historical-payload comparison or a generic late-result arbiter.
...
A **Session stop** selects current work once, including unselected input through its admission cutoff, fences advancement and acknowledges saved intent separately from completion. It excludes that queued prefix through the saved stop fact, even when no Turn is active between failure and successor admission. A stop with no active Turn completes when the exclusion commits; input admitted after its cutoff is not selected by that stop. With an active Turn, completion requires its terminal outcome and atomic occupancy release, including Action settlement and ordered Tool Results; it does not follow future work or prove provider/billing termination. Model transport cleanup may continue under retained custody. A stop cannot rewrite an earlier committed result. The stop atomically binds its core idempotency key, Session reference, selected active Turn (if any), queued admission cutoff and admission answer with the stop authority. A matching retry recovers this selection and answer, including an idle stop, without affecting newer work. Completion observation follows that original selection. A fresh key intentionally requests a new Session stop. Callers retain the key and exact inputs before sending; they need no caller-managed Turn identity. This removes retry-specific Session-reuse coordination, without introducing concurrent Session use as an expected workflow.
...
A definitive inability to continue fails a Turn only after no unresolved Operation/effect, actionable permission or required result publication can change its meaning. Unselected queued messages do not prevent failure and do not inherit its outcome. Atomically save the unique typed outcome for the Turn's selected messages and release Session occupancy. No earlier failure-intent stage is needed. Pre-request failure creates no fake Operation/manifest/Attempt: retain selected configuration revision, input cutoff and canonical frontier as cause, with any selected message bindings under the processing rule above. An Operation-caused failure references that resolved Operation rather than copying evidence. Tool errors, retryable failures, recoverable overflow and direct interruption do not automatically fail the Turn.
...
Inspection derives message `applied` from projection; `not applied` from absent projection plus a selected preparation failure or applicable stop/cancellation authority; otherwise `pending` while unselected. Application means entry into model context, not provider consumption. Keep unapplied content and reason inspectable. Established processing bindings and exclusions are final; untouched admissions remain pending under their original identities across another Turn's failure. A command fixed to the old Turn rejects. Crash alone is neither failure nor cancellation.
...
| The next input boundary selects B before the Turn fails. | B receives that Turn's failure, even if selected preparation fails before projection. It is not automatically queued again. |
...
A **Submission name** is author-provided stable text scoped to one Session within one Workflow. It permits up to 128 UTF-8 bytes of exact text; reject overflow without truncation or normalization. This limit applies to the caller-provided key alone, not the complete core idempotency key. The complete call identity is Workflow ID, exact full Session reference and Submission name; Runtime uses that combination to find the existing intent and its saved core token, or create a new intent with a UUIDv4. The token does not encode the combination: the durable intent already preserves author identity and inputs across the transaction boundary, and core has no consumer for decoded Workflow components. Saving the token there avoids another registry or composite encoding. Configuration and messages share this scope: matching complete identities and inputs recover the original submission, while changed operation kind or inputs conflict. Different Sessions may use the same Submission name, and a new Workflow may reuse an existing Session with the same Submission name for new work. Two references to the same full Session reference share the same scope, regardless of local variable names. Separate intended submissions to that Session within the same Workflow require different Submission names. The core receives only the full Session reference, saved token, operation and inputs; it never needs the submission name or Workflow ID.

Enforce uniqueness of author identity and generated token among Runtime intents. Concurrent equal-identity creation converges on the committed intent after complete input comparison; losers never dispatch discarded candidates. Resolve candidate token collisions before publication with bounded regeneration; entropy failure or exhausted attempts fails new intent creation without weak-random fallback. Existing intents need no new randomness. A committed token is immutable, including after core conflict; never silently replace it. UUID uniqueness is probabilistic: equal-input collisions across independent callers are indistinguishable from retries, while different inputs conflict. No new registry, tuple encoder or lookup round trip is required.
...
The initial implementation uses one asynchronous pull loop. Keep this selection unless a concrete cost justifies a replacement under the [algorithm obligations](#capacity-and-fair-selection).
...
Completed preparation separates canonical input acquisition from child execution; fixed visibility alone does not require copying bodies. Retain this boundary for V1: unused bodies may consume scratch and copying work, but preparation remains memory-bounded and avoids a live coordinator-to-evaluator read protocol inside the child's elapsed lifetime. This is an internal Workflow Runtime boundary; Runtime still obtains core-owned outcomes through ordinary core APIs, and the evaluator receives no core or SQLite access. Add no status or decoded-answer cache. Reconsider narrow immutable range reads if integration demonstrates material preparation delay or scratch exhaustion, preserving fixed visibility, bounded ownership, control service and the existing child deadline.
...
Finish input writes/integrity checks before spawning with completed read-only input. Partial preparation releases its artifacts and starts no child.
...
Classify a connection after its complete request line, then transfer an ordinary route into the 120-place population before releasing one of the eight classification places. Continue the same 16 KiB header buffer and ten-second total header deadline. Accepted control writes publish one process-local changed hint; only the execution owner consumes it, scans the fixed execution slots and rechecks each candidate against canonical Store facts before cancelling transport. The hint carries no command or Operation identity and is never admission, settlement or recovery authority.
...
| Qualification, not admission | Whole Latifa footprint ≤256 MiB at defined 1,000-operation model/Bash/Edit/mixed fixtures, including evaluator/helpers; same cold/retained-idle ceiling. Idle CPU <1% one core. The 60-second, 1,000-stream model reference consumes ≤2 Host cores on average across a conservatively bracketed interval of at least 40 seconds wholly inside simultaneous offered work, and ≤120 Host CPU seconds from before admission through every-key result audit and observed cleanup. p95 durable control acknowledgment ≤1 s; light/free-capacity retry discovery ≤2 s after due. |

### Qualification measurements

Qualify stable memory across churn/history growth. The model CPU fixture offers exactly 1,000 streams ×100 small SSE events/s for 60 seconds, with short requests and bounded terminal bursts. Each stream releases one two-event batch every 20 ms and must complete every batch exactly once. For batch ordinal `i`, completion must not precede release and must occur no later than two intervals after release; adjacent completions may be at most two intervals apart; every half-open 20 ms window may contain at most two completions. These inequalities are the selected workload's explicit timing variation, not an operating-system timing guarantee: they admit one interval beyond the former ordinary deadline while rejecting long pauses and large catch-up bursts. Never skip late work. Stop only on transport/cancellation failure or the offer hard deadline two intervals after 60 seconds.

Exclude the independent fixture process from Latifa totals but record it separately. Bracket the first Host CPU counter read starting no earlier than second 10. Start the second no earlier than both second 50 and 40 seconds after the first read finishes. Use the interval from the first read's finish to the second read's start as the conservative divisor, require that interval to be at least 40 seconds, and require the second read to finish no later than the earliest final-batch completion among valid streams. Report both read brackets and the wider possible interval. Measure complete-work Host CPU from before admission through every-key result audit and observed custody/scratch release; it must not be diluted by an idle tail. Measure tool/mixed CPU separately. Include capture/import contention; report timing violations and cleanup separately from p95 acknowledgment, discovery separately from dispatch. A qualification miss requires explicit implementation/default/target review, not helper exclusion or RSS killing.
...
Reserve growth before I/O with checked arithmetic and serialized owner accounting. Charge sparse gaps and simultaneous copies; overwrites need no new charge. Return only reservation proven unused. When an I/O error does not report a trustworthy written count, retain the complete submitted increment until safe final closure. Private scratch charge survives unlink and pending I/O, releasing only after successful shrink or safe final closure. No writable alias may bypass accounting; no per-write heap allocation, directory scan, SQLite transaction or global lock across disk I/O. Logical bytes do not model block rounding/cache/compression or reserve real disk space.
