...
Concurrent Codex Attempts to one origin share a lazily established HTTP/2 connection population: at most \(\lceil C/100\rceil\) connections for configured Active Capacity \(C\), with a local 100-stream-per-connection limit. Peer limits may queue work inside curl instead of enlarging that population. One easy handle, sealed request and capture remain owned by each Attempt; a connection, TLS session or stream has no semantic authority. Negotiated HTTP/2 is required on HTTPS; the deterministic loopback HTTP/1.1 route stays supported under the same connection bound. The pinned curl patch rejects a completed H2-only TLS handshake without `h2` ALPN before sending a POST; connection, TLS-verification and later H2 stream failures keep their distinct ownership. A reused connection with no response cannot authorize an automatic POST resend; only an HTTP/2 refusal proving no application processing may replay the rewindable sealed request within its original Attempt. Otherwise the ordinary Attempt transport-failure owner decides replacement and allowance. Cancelling or failing one stream leaves siblings independently owned; the reactor alone releases shared connection state. The per-easy upload buffer is 16 KiB; other curl, TLS and paused-stream allocations still need measurement. The execution owner accepts callback bytes into one fixed 16-entry, 16-KiB-per-entry capture queue; a single worker writes and seals the Attempt-owned scratch outside reactor callbacks. A full queue pauses the current curl stream without accepting its bytes; later resumption redelivers them. Cancellation detaches the transport immediately but retains its capture/custody until queued writes drain; settlement and source handoff wait for drainage and successful seal. The queue bounds Rui-owned pending capture bytes to 256 KiB even when disk writes stall, without promising a bound on curl/TLS/socket buffering. Worker blockage may delay capture settlement and graceful shutdown, but not execution-owner control service.
