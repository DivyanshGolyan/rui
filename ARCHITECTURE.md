...
SQLite incremental import must not retain payload-sized dirty-page memory merely because its page cache has a small suggested size. Store initialization explicitly configures spill behavior and verifies the effective cache size, spill threshold, and enforcement of the selected SQLite heap limit on the pinned build. The settings must agree with the approved resource budget; a successful setter alone is not evidence of enforcement. The Storage Owner owns these internal settings, with no caller-facing SQLite tuning surface. The SQLite heap limit is not a whole-Host resident-memory bound. Numeric policy and typed failure behavior remain with the [SQLite resource decision](https://github.com/DivyanshGolyan/onepage/issues/95); verification includes large imports and failed-import rollback.
