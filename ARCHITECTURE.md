...
Every inventory member is represented, but content exposed by a separate public read may remain an immutable reference rather than being copied inline. Evaluator generations/visibility snapshots, physical Attempts, private provider continuation, credentials and internal storage rows are not report collections. Adding another public historical collection requires changing this inventory. Exact command and field spelling remains implementation work.
