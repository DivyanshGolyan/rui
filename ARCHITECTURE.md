...
Human `follow` selects the original saved message key, not whatever Turn is current later. It may return on that message's terminal outcome or an actionable Permission Request requiring attention, even while a sibling Action progresses. Before returning attention from Session Current, it re-observes the saved key: a terminal result takes precedence; otherwise the message must still be queued behind that Session's progress or have a processing binding to Current's Turn. This adds no global snapshot; a later permission decision still validates its exact Action. Its return reason is separate from the authoritative work status: report the progressing sibling as in flight, not the whole Session as blocked. A queued message behind a failed Turn does not inherit that Turn's failure. Closing `follow` changes no work or saved request. This attention condition does not alter the default Session-wait predicate or introduce a Session phase.
