...
The credential owner serializes mutation across Hosts and login processes. Login installation advances a generation. A compact access token with `exp` refreshes near expiry; otherwise the last successful exchange time drives an eight-day proactive refresh, following the pinned client's fallback. The expiry is not inferred from the ID token. Refresh first durably replaces `ready` with `refresh_pending` before contacting the remote issuer, then validates the same account and durably saves the new generation before handoff. A recovered pending refresh requires explicit login: remote token rotation and local file replacement are not one transaction. Same-directory temporary write, file sync, atomic rename and directory sync are one replacement protocol; abandoned private temporary files are never selected as authority. A stable shared lock and immutable record cover the final model launch check, so waiting work selects the latest installed generation and a launched request retains its copied headers through curl teardown. Failed load, refresh or save returns an explicit authentication/backend failure without a model request or alternate backend. Release Rui-owned private buffers and native header copies after their last consumer; native TLS implementation copies are outside that claim. One Host-wide authentication I/O job serves occupied execution slots without file or network waits under Store/Host launch locks.
