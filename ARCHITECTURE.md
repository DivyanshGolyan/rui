...
SQLite's low-water page reserve protects only SQLite closure writes. Admission must also reserve measured immutable-blob bytes and filesystem metadata required to settle every accepted obligation, or prove that a particular closure path requires no new blob. Unreferenced blobs from failed transactions count against that host budget until bounded maintenance reclaims them. Issue #3 owns this combined obligation-backed admission contract.
