...
struct Runtime {
    ...
    int reactor_control_mode;
    _Atomic int reactor_phase;
    int phase_at_commit;
    uint64_t phase_started_ns;
    uint64_t phase_pending_ns[7];
    uint64_t phase_max_ns[7];
    uint64_t reactor_seen_ns;
    uint64_t cancel_remove_ns;
    uint64_t cancel_cleanup_ns;
    unsigned completions_before_cancel;
    unsigned completions_processed;
    int control_mode;
...
#define SETOPT(option, value) \
    ...
    const char *upload_size = getenv("ONEPAGE_PROOF_UPLOAD_BUFFER");
    if (upload_size) SETOPT(CURLOPT_UPLOAD_BUFFERSIZE, strtol(upload_size, NULL, 10));
    const char *body_size = getenv("ONEPAGE_PROOF_REQUEST_BYTES");
    SETOPT(CURLOPT_POSTFIELDSIZE_LARGE, (curl_off_t)(body_size ? strtoll(body_size, NULL, 10) : 4096));
...
static void reactor_phase(struct Runtime *runtime, int next) {
    uint64_t end = now_ns();
    int old = atomic_load(&runtime->reactor_phase);
    uint64_t start = runtime->phase_started_ns;
    if (start) {
        uint64_t elapsed = end - start;
        if (elapsed > runtime->phase_max_ns[old]) runtime->phase_max_ns[old] = elapsed;
        if (atomic_load(&runtime->cancel_target)) {
            uint64_t low = start > runtime->control_committed_ns ? start : runtime->control_committed_ns;
            uint64_t removed = atomic_load(&runtime->control_removed_ns);
            uint64_t high = removed && removed < end ? removed : end;
            if (high > low) runtime->phase_pending_ns[old] += high - low;
        }
    }
    runtime->phase_started_ns = end;
    atomic_store(&runtime->reactor_phase, next);
}
static int reactor_cancel(struct Runtime *runtime, CURLM *multi) {
    uint64_t target = atomic_load(&runtime->cancel_target);
    if (!target || atomic_load(&runtime->control_removed_ns)) return 0;
    runtime->reactor_seen_ns = now_ns();
    runtime->completions_before_cancel = runtime->completions_processed;
    int previous_phase = atomic_load(&runtime->reactor_phase);
    reactor_phase(runtime, PH_CANCEL);
    struct PhysicalCustody *cell = &runtime->cells[runtime->capacity - 1];
    if (cell->operation_id != target || atomic_load(&cell->state) != CUSTODY_ACTIVE || !cell->easy) return -1;
    uint64_t started = now_ns();
    if (curl_multi_remove_handle(multi, cell->easy) != CURLM_OK) return -1;
    runtime->cancel_remove_ns = now_ns() - started;
    started = now_ns();
    curl_easy_cleanup(cell->easy); cell->easy = NULL;
    runtime->cancel_cleanup_ns = now_ns() - started;
    cell->flags |= FLAG_CANCELLED;
    atomic_store(&runtime->control_removed_ns, now_ns());
    seal_cell(runtime, cell);
    reactor_phase(runtime, previous_phase);
    return 0;
}
...
static void *reactor_main(void *opaque) {
    ...
    while (!atomic_load_explicit(&runtime->stop, memory_order_acquire)) {
        ...
        reactor_phase(runtime, PH_COMPLETIONS);
        int remaining = 0;
        CURLMsg *message;
        while (true) {
            if (runtime->reactor_control_mode >= 2 && reactor_cancel(runtime, multi) != 0) {
            ...
            for (int i = 0; i < runtime->capacity; ++i) {
                ...
                cell->easy = NULL;
                runtime->completions_processed++;
                seal_cell(runtime, cell);
                break;
            }
        }
        if (atomic_load(&runtime->fatal) || reactor_cancel(runtime, multi) != 0) {
            atomic_store(&runtime->fatal, true); break;
        }
        reactor_phase(runtime, PH_OTHER);
        for (int i = 0; i < runtime->capacity; ++i) {
            struct PhysicalCustody *cell = &runtime->cells[i];
            if (cell->kind == KIND_BASH &&
...
static int run_integrated(int argc, char **argv) {
    ...
    uint64_t removed = atomic_load(&runtime.control_removed_ns);
    printf("{\"reactor_control\":{\"mode\":%d,\"phase_at_commit\":%d,\"seen_ns\":%llu,\"remove_call_ns\":%llu,\"cleanup_call_ns\":%llu,\"completions_before_cancel\":%u,\"pending_phase_ns\":[",
        runtime.reactor_control_mode, runtime.phase_at_commit, runtime.reactor_seen_ns,
        runtime.cancel_remove_ns, runtime.cancel_cleanup_ns, runtime.completions_before_cancel);
    for (int i=0; i<7; ++i) printf("%s%llu", i ? "," : "", runtime.phase_pending_ns[i]);
    printf("],\"max_phase_ns\":[");
    for (int i=0; i<7; ++i) printf("%s%llu", i ? "," : "", runtime.phase_max_ns[i]);
    printf("]},");
