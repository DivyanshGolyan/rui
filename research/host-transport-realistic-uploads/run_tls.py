...
import shlex, argparse, hashlib, json, os, pathlib, platform, resource, select, shutil, signal, sqlite3, subprocess, sys, tempfile, time
P=pathlib.Path(__file__).resolve().parent
p=argparse.ArgumentParser(); p.add_argument('--smoke',action='store_true'); p.add_argument('--capacity',type=int,choices=[100,500,1000]); p.add_argument('--curl-build',type=pathlib.Path); a=p.parse_args()
...
if a.curl_build:
    ...
    link = shlex.split(subprocess.check_output(['sh',str(a.curl_build/'curl-config'),'--static-libs'],text=True))
    link = [str(a.curl_build/'lib/.libs/libcurl.a') if item.endswith('/libcurl.a') else item for item in link] + ['-lsqlite3','-lpthread']
meta={'date':time.strftime('%Y-%m-%dT%H:%M:%S%z'),'platform':platform.platform(),'python':sys.version,
      'curl_build':str(a.curl_build),'link_flags':link,'fd_limit':resource.getrlimit(resource.RLIMIT_NOFILE),'flags':flags,
      ...
      'source_hashes':{f:hashlib.sha256((P/f).read_bytes()).hexdigest() for f in ('network.c','server.py','run_tls.py')},
...
output=P/('tls-smoke.json' if a.smoke else ('tls-single.json' if a.capacity else 'tls-results.json'))
...
with tempfile.TemporaryDirectory(prefix='onepage-capacity-PROTOTYPE-') as tmp:
    ...
    subprocess.run(['clang',*flags,str(P/'network.c'),'-o',str(binary),*link],check=True)
    ...
    cases=[(10,16384,262144)] if a.smoke else [(1000,b,n) for n in (262144,1048576,4194304) for b in ((65536,16384) if n!=1048576 else (16384,65536))]
    for index,(capacity,upload_buffer,request_bytes) in enumerate(cases):
        reactor_mode=3
        cycles,seconds,control = 1, (1 if a.smoke else 4), 3
        case=root/str(index); case.mkdir(); server=None; client=None
        row={'index':index,'capacity':capacity,'cycles':cycles,'seconds':seconds,'control_mode':control,'reactor_mode':reactor_mode,'upload_buffer':upload_buffer,'request_bytes':request_bytes}
        try:
            server=subprocess.Popen([sys.executable,str(P/'server.py'),'--cert',str(cert),'--key',str(key),'--capacity',str(capacity),'--cycles',str(cycles),'--seconds',str(seconds),'--request-bytes',str(request_bytes),*(['--control'] if control else [])],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
            ...
            env['ONEPAGE_PROOF_REACTOR_CONTROL']=str(reactor_mode)
            env['ONEPAGE_PROOF_UPLOAD_BUFFER']=str(upload_buffer)
            env['ONEPAGE_PROOF_REQUEST_BYTES']=str(request_bytes)
