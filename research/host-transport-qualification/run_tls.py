...
with tempfile.TemporaryDirectory(prefix='onepage-capacity-PROTOTYPE-') as tmp:
    ...
    cases=[(10,3)] if a.smoke else [(c,3) for c in ([a.capacity] if a.capacity else [100,500,1000])]
    for index,(capacity,reactor_mode) in enumerate(cases):
        ...
        row={'index':index,'capacity':capacity,'cycles':cycles,'seconds':seconds,'control_mode':control,'reactor_mode':reactor_mode}
        try:
