...
with open('/tmp/onepage-memory-experiments.lock','a') as lock:
 ...
 for case in cases:
  name,n,r,o,items,mode,cache,buf,proto=case
  ...
  with open(out/(name+'-server.jsonl'),'w') as serverlog,open(out/(name+'-server.stderr'),'w') as err:
   ...
   try:
    ...
    argv=[str(b/'probe'),str(n),str(r),str(o),str(items),str(mode),str(cache),str(buf),str(proto==2 and 1 or 0),f'https://localhost:{port}/',str(cert)]
