...
async def accept(reader,writer):
 ...
 resumed+=int(reused)
 print(json.dumps({'connection':cid,'resumed':reused}),flush=True)
 try:
  if proto==1:
   while True:
    ...
    for off in range(0,size,16384):
     ...
     writer.write(b'x'*min(size-off,16384));await writer.drain()
  else:
   ...
   hashes={};sizes={};paths={};pending={};ended={}
   from h2.events import RequestReceived
   async def pump():
    warmed=set();release=None
    while True:
     for sid,(burst,ready) in list(ended.items()):
      if barriers[burst].is_set() and asyncio.get_running_loop().time()>=ready:
       h.send_headers(sid,[(':status','200'),('content-length',str(size))]);pending[sid]=size;del ended[sid]
     for sid,left in list(pending.items()):
      if warm and left<=size-warm:
       warmed.add(sid)
       if len(warmed)==n and release is None:release=asyncio.get_running_loop().time()+1
    ...
    while data:=await reader.read(65536):
     for ev in h.receive_data(data):
      if isinstance(ev,RequestReceived):paths[ev.stream_id]=int(dict(ev.headers)[b':path'].strip(b'/'))
       ...
       burst=paths.pop(sid);complete(burst);ended[sid]=(burst,asyncio.get_running_loop().time()+.1)
