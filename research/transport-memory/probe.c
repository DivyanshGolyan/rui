...
// OS metrics.
static _Atomic size_t live[2], peak[2], blocks[2], largest[2], biglive[2],
    bigpeak[2];
static void size_observation(size_t n, size_t old, int k) {
  size_t high = largest[k];
  while (n > high && !atomic_compare_exchange_weak(&largest[k], &high, n)) {
  }
  const size_t threshold = 8 * 1024 * 1024;
  if (n >= threshold && old < threshold) {
    size_t count = ++biglive[k];
    high = bigpeak[k];
    while (count > high &&
           !atomic_compare_exchange_weak(&bigpeak[k], &high, count)) {
    }
  } else if (n < threshold && old >= threshold)
    --biglive[k];
}
...
static void freex(void *p) {
  if (!p)
    return;
  H *h = (H *)p - 1;
  size_observation(0, h->x.n, h->x.k);
  live[h->x.k] -= h->x.n;
  blocks[h->x.k]--;
  free(h);
}
...
int main(int ac, char **av) {
  ...
  int h2 = atoi(av[8]);
  long upload = ac == 12 ? atol(av[11]) : 0;
  assert(n > 0 && n <= 1000 && items > 0 && reqbytes >= (size_t)items);
  assert(reqbytes <= 16777216 && response_bytes <= 16777216);
  assert((size_t)n * (reqbytes + response_bytes) <= 256 * 1024 * 1024);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (upload)
      CE(curl_easy_setopt(t->e, CURLOPT_UPLOAD_BUFFERSIZE, upload));
  ...
  printf("{\"curl_largest_allocation\":%zu,\"tls_largest_allocation\":%zu,"
         "\"curl_peak_live_allocations_ge8MiB\":%zu}\n",
         largest[0], largest[1], bigpeak[0]);
