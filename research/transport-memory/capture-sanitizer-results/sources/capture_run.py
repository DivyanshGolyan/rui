...
for protocol in [1,2]:
 for stall,label in [(0,'large-control'),(3000,'late-stall')]:cases.append((f'h{protocol}-worker-{label}',1000,protocol,1,393216,2,0,stall,16,16,0,131072))
 ...
 for case in cases:
  ...
  with tempfile.TemporaryDirectory(prefix='onepage-capture-',dir='/tmp') as tmp:
   ...
   with open(out/(name+'-server.jsonl'),'w') as slog,open(out/(name+'-server.stderr'),'w') as serr,open(out/(name+'.stderr'),'w') as cerr:
    server=subprocess.Popen([str(b/'venv/bin/python'),str(HERE/'capture_server.py'),str(n),str(payload),str(bursts),str(protocol),str(port),str(scratch/'cert.pem'),str(scratch/'key.pem'),str(warm)],stdout=slog,stderr=serr,preexec_fn=limits)
    client=control=None
    try:
     for _ in range(200):
     ...
     argv=[str(binary),str(n),str(protocol),str(bursts),str(payload),str(strategy),str(delay),str(stall),str(slots),str(cache),f'https://localhost:{port}',tmp,str(fail),str(warm)]
