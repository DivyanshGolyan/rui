...
static void capture_write(struct Capture *t, const void *bytes, size_t length) {
  double begin = now();
  if (sink.stall_ms && !atomic_exchange(&sink.stalled, 1))
  ...
  /* Only the capture owner updates this; main reads after queue drain/join. */
...
int main(int argc, char **argv) {
  ...
  assert(n > 0 && n <= 1000 && bursts > 0 && bursts <= 4 &&
         payload <= 16777216 && (payload + 1024) * n <= 256 * 1024 * 1024);
