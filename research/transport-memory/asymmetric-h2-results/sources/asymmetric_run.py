...
if a.quick:cases=[case('asymmetric-h1-smoke',1,16384,1048576,mixed=1,stall=3000,delay_ms=10,up=4194304,n=8)]
...
with open('/tmp/onepage-memory-experiments.lock','a') as lock:
 ...
 for case in cases:
  ...
  warm,rate,receive,hosts,streams,servercap,known,tokens,events,hz,upload,request_base,growth,mixed,up_rate,down_rate,delay_ms=extra
  with tempfile.TemporaryDirectory(prefix='onepage-capture-',dir='/tmp') as tmp:
   ...
   with open(out/(name+'-server.jsonl'),'w') as slog,open(out/(name+'-server.stderr'),'w') as serr,open(out/(name+'.stderr'),'w') as cerr,open(out/(name+'-proxy.jsonl'),'w') as plog,open(out/(name+'-proxy.stderr'),'w') as perr:
    ...
    try:
     ...
     argv=[str(binary),str(n),str(protocol),str(bursts),str(payload),str(strategy),str(delay),str(stall),str(slots),str(cache),f'https://localhost:{proxy_port}',tmp,str(fail),str(warm),str(rate),str(receive),str(hosts),str(streams),str(tokens),str(events),str(upload),str(request_base),str(growth),str(mixed)]
