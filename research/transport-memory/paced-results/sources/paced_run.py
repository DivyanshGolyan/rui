 ...
 for case in cases:
  ...
  with tempfile.TemporaryDirectory(prefix='onepage-capture-',dir='/tmp') as tmp:
   ...
   with open(out/(name+'-server.jsonl'),'w') as slog,open(out/(name+'-server.stderr'),'w') as serr,open(out/(name+'.stderr'),'w') as cerr:
    server=subprocess.Popen([str(b/'venv/bin/python'),str(HERE/'paced_server.py'),str(n),str(tokens),str(events),str(hz),str(port),str(scratch/'cert.pem'),str(scratch/'key.pem')],stdout=slog,stderr=serr,preexec_fn=limits)
    client=control=None
    try:
     for _ in range(200):
