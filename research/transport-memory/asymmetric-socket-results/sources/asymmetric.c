...
static size_t upload_source_bytes, staged_bytes, staging_read_bytes;
static int socket_send_buffer, socket_buffer_count, socket_buffer_min = INT_MAX,
                                                    socket_buffer_max;
static int configure_socket(void *unused, curl_socket_t fd,
                            curlsocktype purpose) {
  (void)unused;
  assert(purpose == CURLSOCKTYPE_IPCXN);
  if (socket_send_buffer) {
    if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &socket_send_buffer,
                   sizeof(socket_send_buffer)))
      return CURL_SOCKOPT_ERROR;
    int actual;
    socklen_t length = sizeof(actual);
    if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &actual, &length))
      return CURL_SOCKOPT_ERROR;
    socket_buffer_count++;
    if (actual < socket_buffer_min)
      socket_buffer_min = actual;
    if (actual > socket_buffer_max)
      socket_buffer_max = actual;
  }
  return CURL_SOCKOPT_OK;
}

static const char request_prefix[] =
    "{\"model\":\"fixture\",\"input\":[{\"role\":\"user\",\"content\":[{"
    "\"type\":\"input_text\",\"text\":\"";
static const char request_suffix[] = "\"}]}]}";
static void request_block(char *out, size_t offset, size_t length,
                          size_t total) {
  memset(out, 'a', length);
  for (size_t i = 0; i < length; i++) {
    size_t pos = offset + i;
    if (pos < sizeof(request_prefix) - 1)
      out[i] = request_prefix[pos];
    else if (pos >= total - (sizeof(request_suffix) - 1))
      out[i] = request_suffix[pos - (total - (sizeof(request_suffix) - 1))];
  }
}
...
struct Capture {
  ...
  size_t accepted, request_bytes;
  uint64_t first_read_ns, last_read_ns, first_body_ns;
...
static void phase(const char *name) {
  ...
  struct rusage_info_v4 disk = {0};
  assert(!proc_pid_rusage(getpid(), RUSAGE_INFO_V4, (rusage_info_t *)&disk));
  printf("{\"disk_physical_sample\":\"%s\",\"burst\":%d,\"read\":%llu,"
         "\"written\":%llu}\n",
         name, burst_id, (unsigned long long)disk.ri_diskio_bytesread,
         (unsigned long long)disk.ri_diskio_byteswritten);
  struct rusage usage;
...
static size_t capture_callback(char *bytes, size_t a, size_t b, void *ctx) {
  ...
  if (length && !t->first_body_ns)
    t->first_body_ns = nano();
  assert(length <= CHUNK);
  if (t->failed)
...
static size_t request_read(char *bytes, size_t a, size_t b, void *ctx) {
  ...
  if (length > 0) {
    if (!t->first_read_ns)
      t->first_read_ns = nano();
    t->last_read_ns = nano();
    upload_source_bytes += length;
  }
...
static void finish_handle(struct Capture *t, CURLM *m, CURLcode result,
  ...
  long nc = 0, status = 0;
  curl_off_t post = 0, first = 0, total = 0, upload = 0;
  CE(curl_easy_getinfo(t->easy, CURLINFO_POSTTRANSFER_TIME_T, &post));
  CE(curl_easy_getinfo(t->easy, CURLINFO_STARTTRANSFER_TIME_T, &first));
  CE(curl_easy_getinfo(t->easy, CURLINFO_TOTAL_TIME_T, &total));
  CE(curl_easy_getinfo(t->easy, CURLINFO_SIZE_UPLOAD_T, &upload));
  if (result != CURLE_OK) {
    long os_error = 0;
    CE(curl_easy_getinfo(t->easy, CURLINFO_OS_ERRNO, &os_error));
    fprintf(stderr, "curl failure %d, OS error %ld: %s\n", (int)result,
            os_error, t->error);
    printf("{\"transport_failure\":%d,\"curl_code\":%d,\"upload_bytes\":%lld,"
           "\"expected_request_bytes\":%zu}\n",
           t->id, (int)result, (long long)upload, t->request_bytes);
    fflush(stdout);
  }
  assert((size_t)upload == t->request_bytes);
  printf(
      "{\"request_timing\":%d,\"burst\":%d,\"upload_bytes\":%lld,"
      "\"posttransfer_us\":%lld,\"starttransfer_us\":%lld,\"total_us\":%lld,"
      "\"first_read_ns\":%llu,\"last_read_ns\":%llu,\"first_body_ns\":%llu}\n",
      t->id, burst_id, (long long)upload, (long long)post, (long long)first,
      (long long)total, (unsigned long long)t->first_read_ns,
      (unsigned long long)t->last_read_ns,
      (unsigned long long)t->first_body_ns);
  ...
  *connections += nc;
  ...
  scratchbytes -= t->request_bytes;
  active_transfers--;
}
int main(int argc, char **argv) {
  assert(argc == 24 || argc == 25);
  socket_send_buffer = argc == 25 ? atoi(argv[24]) : 0;
  assert(socket_send_buffer >= 0);
  long upload_buffer = atol(argv[20]);
  size_t request_base = strtoull(argv[21], NULL, 10);
  int growth = atoi(argv[22]), mixed = atoi(argv[23]);
  ...
  assert(n > 0 && n <= 1000 && bursts > 0 && bursts <= 4 &&
         payload <= 16777216 &&
         (payload + request_base * (growth ? 16 : 1)) * n <=
  ...
  for (burst_id = 0; burst_id < bursts; burst_id++) {
    ...
    sink.written = 0;
    upload_source_bytes = staged_bytes = staging_read_bytes = 0;
    sink.stop = 0;
    sink.stalled = 0;
    ...
    for (int i = 0; i < n; i++) {
      ...
      t->id = i;
      static const size_t factors[] = {1, 4, 16, 4};
      t->request_bytes = request_base * (growth ? factors[burst_id] : 1);
      if (mixed && i % 2)
        t->request_bytes /= 64;
      assert(t->request_bytes >= 1024);
      assert(snprintf(url, sizeof(url), "%s/%d/%d", base, burst_id, i) <
             (int)sizeof(url));
      t->request_fd = scratch();
      t->capture_fd = scratch();
      for (size_t off = 0; off < t->request_bytes;) {
        size_t length = t->request_bytes - off;
        if (length > sizeof(block))
          length = sizeof(block);
        request_block(block, off, length, t->request_bytes);
        assert(!writeall(t->request_fd, block, length));
        off += length;
        staged_bytes += length;
        control_service();
      }
      ...
      for (size_t off = 0; off < t->request_bytes;) {
        ...
        assert(read(t->request_fd, verify, length) == (ssize_t)length);
        request_block(block, off, length, t->request_bytes);
        assert(!memcmp(verify, block, length));
        off += length;
        staging_read_bytes += length;
        control_service();
      }
      assert(lseek(t->request_fd, 0, SEEK_SET) == 0);
      scratchbytes += t->request_bytes;
      ...
      assert(t->easy);
      CE(curl_easy_setopt(t->easy, CURLOPT_SOCKOPTFUNCTION, configure_socket));
      CE(curl_easy_setopt(t->easy, CURLOPT_ERRORBUFFER, t->error));
      ...
      CE(curl_easy_setopt(t->easy, CURLOPT_UPLOAD_BUFFERSIZE, upload_buffer));
    ...
    while (done < n || queue_count()) {
      ...
      if (now() - last > .1) {
        ...
        for (int i = 0; i < n; i++)
          if (transfers[i].easy)
            scratchbytes += transfers[i].request_bytes;
        phase("active");
        last = now();
      }
    ...
    printf("{\"socket_buffer_burst\":%d,\"requested\":%d,\"configured_"
           "connections\":%d,\"initial_min\":%d,\"initial_max\":%d}\n",
           burst_id, socket_send_buffer, socket_buffer_count,
           socket_buffer_count ? socket_buffer_min : 0, socket_buffer_max);
    assert(upload_source_bytes == staged_bytes &&
           staging_read_bytes == staged_bytes);
    printf("{\"disk_io_burst\":%d,\"staging_written\":%zu,\"staging_verified_"
           "read\":%zu,\"upload_read\":%zu,\"capture_written\":%zu}\n",
           burst_id, staged_bytes, staging_read_bytes, upload_source_bytes,
           (size_t)sink.written);
