...
p=argparse.ArgumentParser();p.add_argument('--groups',type=pathlib.Path,nargs='+',required=True);p.add_argument('--output',type=pathlib.Path,required=True);a=p.parse_args();rows=[]
...
for group in a.groups:
 ...
 for text in (group/'summary.jsonl').read_text().splitlines():
  ...
  assert name not in seen;seen.add(name);assert rec['returncode']==rec['control_returncode']==0
  ...
  if protocol==2:assert server[-1]['streams_max']==n
  ...
  for burst in range(bursts):
   ...
   rows.append({'group':str(group),'case':name,'burst':burst,'n':n,'protocol':protocol,'strategy':strategy,'delay_us':delay,'stall_ms':stall,'slots':slots,'cache':cache,'capture_seconds':elapsed,'verified_bytes':expected,'verified_MiB_s':expected/1048576/elapsed,'control_samples':len(latency),'control_p95_ms':q(latency,.95),'control_max_ms':max(latency),'generator_lateness_max_ms':max(sends[r['ack']]['schedule_lateness_ns']/1e6 for r in loaded),'callback_max_ms':finish['callback_max_s']*1000,'perform_max_ms':finish['perform_max_s']*1000,'unpause_max_ms':finish['unpause_max_s']*1000,'write_max_ms':finish['write_max_s']*1000,'queue_peak_bytes':finish['queue_peak_bytes'],'curl_peak':max(r['curl_peak'] for r in active[burst]),'tls_peak':max(r['tls_peak'] for r in active[burst]),'process_physical_highwater':max(r['max_physical'] for r in active[burst]),'idle_physical':idle['physical'],'idle_curl_live':idle['curl_live'],'idle_tls_live':idle['tls_live'],'idle_fds':idle['fds'],'new_connections':new,'resumed_new_connections':reused_sessions,'full_handshakes':new-reused_sessions,'new_connection_setup_mean_ms':b['new_connection_appconnect_us_sum']/max(1,new)/1000,'handshake_phase_mean_ms':b['handshake_phase_us_sum']/max(1,new)/1000,'cpu_seconds':cpu,'mean_cpu_cores':cpu/elapsed})
