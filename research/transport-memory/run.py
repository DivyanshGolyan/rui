...
binary=b/('probe-'+hashlib.sha256(str(out).encode()).hexdigest()[:12])
ssl=pathlib.Path('/opt/homebrew/Cellar/openssl@3/3.6.3')
compile=['cc','-O2','-Wall','-Wextra','-Wno-deprecated-declarations','-DCURL_STATICLIB','-I'+str(b/'curl-8.22.0/include'),'-I'+str(ssl/'include'),str(HERE/'probe.c'),str(b/'curl-8.22.0/lib/.libs/libcurl.a'),str(b/'ng/lib/libnghttp2.a'),'-L'+str(ssl/'lib'),'-lssl','-lcrypto','-lz','-framework','CoreFoundation','-framework','CoreServices','-framework','Security','-framework','SystemConfiguration','-o',str(binary)]
if args.sanitize:compile[1:1]=['-fsanitize=address,undefined','-fno-omit-frame-pointer']
subprocess.run(compile,check=True)
cert=b/(binary.name+'-cert.pem');key=b/(binary.name+'-key.pem')
subprocess.run([str(ssl/'bin/openssl'),'req','-x509','-newkey','rsa:2048','-nodes','-keyout',str(key),'-out',str(cert),'-days','2','-subj','/CN=localhost','-addext','subjectAltName=DNS:localhost,IP:127.0.0.1'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=True)
meta={'recorded_at_utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'dependency_sha256':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [b/'curl-8.22.0/lib/.libs/libcurl.a',b/'ng/lib/libnghttp2.a',ssl/'lib/libssl.3.dylib',ssl/'lib/libcrypto.3.dylib']},'revision':cmd(['git','rev-parse','HEAD']),'machine':platform.uname()._asdict(),'memory_bytes':cmd(['sysctl','-n','hw.memsize']),'compiler':cmd(['cc','--version']),'compile':compile,'openssl':cmd([str(ssl/'bin/openssl'),'version','-a']),'dependencies':cmd([str(b/'venv/bin/pip'),'freeze']),'linked':cmd(['otool','-L',str(binary)]),'sources_sha256':{f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in [HERE/'probe.c',HERE/'server.py',HERE/'run.py',HERE/'build.py']},'curl_config':(b/'curl-8.22.0/libcurl.pc').read_text(),'nghttp2_sha256':'e05cb1388eaca3830aded4ccf20044b6e1ac1a61411dcca11b0437c4285c8bc2','curl_sha256':'f7ef3ae8a22e521f289803fe93543eb64c329b58aa73a9e224dfd915a2a5f4f7'}
 ...
 for case in cases:
  ...
  with open(out/(name+'-server.jsonl'),'w') as serverlog,open(out/(name+'-server.stderr'),'w') as err:
   ...
   try:
    ...
    argv=[str(binary),str(n),str(r),str(o),str(items),str(mode),str(cache),str(buf),str(proto==2 and 1 or 0),f'https://localhost:{port}/',str(cert),str(upload)]
