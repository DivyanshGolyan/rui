...
cases=[]
# warm, receive rate, shared read buffer request, host connections, local stream cap, server stream cap
for label,rate,receive,hosts,streams,servercap in [
 ('baseline',0,16384,1,1000,1000),('recv1k',0,1024,1,1000,1000),
 ('rate32k',32768,16384,1,1000,1000),('rate128k',131072,16384,1,1000,1000),
 ('split10',0,16384,10,100,1000),('peer100',0,16384,10,1000,100)]:
 for stall,suffix in [(0,'burst'),(3000,'late')]:
  cases.append((f'h2-{label}-{suffix}',1000,2,1,393216,2,0,stall,16,16,0,131072,rate,receive,hosts,streams,servercap))
if a.quick:cases=[('h2-adjustment-smoke',8,2,1,393216,2,0,3000,16,16,0,131072,131072,16384,2,4,4)]
...
with open('/tmp/onepage-memory-experiments.lock','a') as lock:
 ...
 for case in cases:
  ...
  warm,rate,receive,hosts,streams,servercap=extra
  with tempfile.TemporaryDirectory(prefix='onepage-capture-',dir='/tmp') as tmp:
   ...
   with open(out/(name+'-server.jsonl'),'w') as slog,open(out/(name+'-server.stderr'),'w') as serr,open(out/(name+'.stderr'),'w') as cerr:
    server=subprocess.Popen([str(b/'venv/bin/python'),str(HERE/'adjustment_server.py'),str(n),str(payload),str(bursts),str(protocol),str(port),str(scratch/'cert.pem'),str(scratch/'key.pem'),str(warm),str(servercap)],stdout=slog,stderr=serr,preexec_fn=limits)
    client=control=None
    try:
     for _ in range(200):
