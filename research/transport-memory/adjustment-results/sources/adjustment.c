...
int main(int argc, char **argv) {
  ...
  assert(n > 0 && n <= 1000 && bursts > 0 && bursts <= 4 &&
         payload <= 16777216 && (payload + 1024) * n <= 512 * 1024 * 1024);
  ...
  for (burst_id = 0; burst_id < bursts; burst_id++) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      while ((got = read(t->capture_fd, block, sizeof(block))) > 0) {
        for (size_t j = 0; j < (size_t)got;) {
          size_t index = (bytes + j) / 256, pos = (bytes + j) % 256;
          char event[256];
          int len;
          if (index == payload / 256 - 1)
            len = snprintf(event, sizeof(event),
                           "event: response.completed\ndata: "
                           "{\"sequence\":\"%05zu\",\"status\":\"completed\"}",
                           index);
          else
            len = snprintf(event, sizeof(event),
                           "event: response.output_text.delta\ndata: "
                           "{\"sequence\":\"%05zu\",\"delta\":\"fixture\"}",
                           index);
          assert(len > 0 && len < 254);
          memset(event + len, ' ', 254 - len);
          event[254] = event[255] = '\n';
          size_t take = 256 - pos;
          if (take > (size_t)got - j)
            take = (size_t)got - j;
          assert(!memcmp(block + j, event + pos, take));
          j += take;
        }
        bytes += got;
        control_service();
