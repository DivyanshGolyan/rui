...
with open('/tmp/rui-memory-experiments.lock','a') as lock:
 ...
 with tempfile.TemporaryDirectory(prefix='rui-evaluator-production-') as tmp:
  ...
  if result.returncode==0:
   exe=prefix/'bin/rui-workflow-evaluator';out['executable_sha256']=hashlib.sha256(exe.read_bytes()).hexdigest()
