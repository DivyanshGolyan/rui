...
with tempfile.TemporaryDirectory(prefix='onepage-capacity-PROTOTYPE-') as tmp:
    ...
    cases=[(3,1,1,False)] if a.smoke else [(c,1,4,False) for order in [(100,500,1000),(500,1000,100),(1000,100,500)] for c in order]+[(1000,5,4,False),(1000,5,4,True),(1000,1,20,False)]
    if a.capacity: cases=[(a.capacity,1,4,False)]
    for index,(capacity,cycles,seconds,relief) in enumerate(cases):
        case=root/str(index); case.mkdir(); server=None; client=None
        row={'index':index,'capacity':capacity,'cycles':cycles,'seconds':seconds,'relief':relief}
        try:
            server=subprocess.Popen([sys.executable,str(P/'server.py'),'--cert',str(cert),'--key',str(key),'--capacity',str(capacity),'--cycles',str(cycles),'--seconds',str(seconds)],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
            ...
            if relief: env['ONEPAGE_PROOF_PRESSURE_RELIEF']='1'
            ...
            row['checks']={
                'client_success':client.returncode==0 and c['completion_classes']['success']==n and c['resolution_rows']==n and not c['host_fatal'],
                'full_cohorts':s['accepted']==n and s['completed']==n and s['disconnected']==0 and len(s['waves'])==cycles and all(w['concurrent']==capacity for w in s['waves']),
                'exact_bytes':c['received_bytes']==s['body_bytes']==s['expected_body_bytes']==stored,
