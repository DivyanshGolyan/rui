...
async def main(a):
    ...
    stats = dict(accepted=0, completed=0, disconnected=0, body_bytes=0, waves=[])
    ...
    async def handle(reader, writer):
        try:
            ...
            assert length == 4096
            await asyncio.wait_for(reader.readexactly(length), 30)
    ...
    stats.update(wall_seconds=time.monotonic()-start,cpu_seconds=u.ru_utime+u.ru_stime,max_rss_bytes=u.ru_maxrss,
        expected_body_bytes=a.capacity*a.cycles*(a.seconds*100*len(delta)+len(terminal)))
