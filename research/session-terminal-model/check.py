...
CASES = {
    'correct': None,
    'progress': None,
    'broken-carry': 'ExcludedNeverApplied',
    'broken-disposition': 'ExcludedNeverApplied',
    'broken-late-result': 'NoLateAcceptance',
    'witness-failed-continuation': 'NoFailedContinuationWitness',
    'witness-stopped-continuation': 'NoStoppedContinuationWitness',
    'witness-crash-recovery': 'NoCrashRecoveryWitness',
    'witness-late-after-continuation': 'NoLateAfterContinuationWitness',
}
...
def main():
    ...
    for name, expected in CASES.items():
        with tempfile.TemporaryDirectory(prefix='onepage-session-tlc-') as tmp:
            ...
            work = Path(tmp)
            for filename in ['SessionTerminal.tla', name + '.cfg']:
