...
with tempfile.TemporaryDirectory(prefix='onepage-capacity-PROTOTYPE-') as tmp:
    ...
    cases=[(10,16384)] if a.smoke else [(1000,b) for b in (65536,16384,16384,65536,65536,16384)]
    for index,(capacity,upload_buffer) in enumerate(cases):
        reactor_mode=3
        cycles,seconds,control = 1, (1 if a.smoke else 4), 3
        case=root/str(index); case.mkdir(); server=None; client=None
        row={'index':index,'capacity':capacity,'cycles':cycles,'seconds':seconds,'control_mode':control,'reactor_mode':reactor_mode,'upload_buffer':upload_buffer}
        try:
