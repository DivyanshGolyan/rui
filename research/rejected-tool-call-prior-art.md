...
| Waiting until every call has a rejection result or resolved Action, publishing Tool Results once in original order and admitting the next model Operation | Continuation owner introduced in #199 | Revalidating against current settings or physical completion order |
...
4. **Issue ownership follows the facts.** #198 owns envelope validation, frozen-catalog classification, atomic rejection/Action admission, observation and denial. #199 owns ordered publication across mixed rejected calls and resolved Actions. #200 receives only exact authorized executable descriptors and owns Attempt admission and external-effect uncertainty.
