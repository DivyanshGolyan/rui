...
## Closed-invocation follow-up

`python3 research/patch-scratch-probe/invocation.py` checks eight environment/attribute cases on each installed build; all 16 preserved exact expected output bytes. Cases include ambient global CRLF configuration/attributes, local CRLF, editing `.gitattributes` itself, local and self-declared UTF-16 encoding, ident expansion, and a parent Git repository. `invocation-results.json` contains the observations. Local-attribute fixtures deliberately contain an extra file to challenge the invocation; production scratch contains only the exact one-file snapshot and path directories.

The selected candidate uses an explicit environment (no inherited Git variables), `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_ATTR_NOSYSTEM=1`, and `GIT_CEILING_DIRECTORIES` naming the controlled scratch parent. Invocation uses `-c core.attributesFile=/dev/null -c core.autocrlf=false apply --no-index --whitespace=nowarn -`. Keep scratch outside a repository, create no `.git`, and prevent repository discovery above the scratch parent. The two builds preserved literal text even for self-targeted `.gitattributes`; no working-tree conversion is part of this Patch behavior. Qualify that behavior again on the selected release build rather than promising it for arbitrary Git versions.

For this closed one-file, ordinary-text/no-conversion invocation, each added byte must occur in the input patch, so S+P is a conservative output bound. Checked arithmetic and child RLIMIT_FSIZE enforce the bound. Only one replacement is emitted; before it, Git removes the old regular-file path, whose unused handle OnePage has already closed. Flags enabling indexes, binary changes, multiple paths, rejected hunk files, renames/copies, creation/deletion or mode changes remain excluded by the existing Patch preflight. A directory of unexpected shape is a failure, not permission for arbitrary helper writes. No additional user-selected byte quota is introduced.

This follow-up resolves the design's invocation/conversion concern for the selected candidate and tested builds. It does not replace production grammar, descriptor, fault/recovery or whole-Host qualification. Source rationale also includes [Git v2.39.5 attr.c](https://github.com/git/git/blob/v2.39.5/attr.c) and [convert.c](https://github.com/git/git/blob/v2.39.5/convert.c); the documented [attribute sources](https://git-scm.com/docs/gitattributes) motivate explicitly excluding ambient attributes and configuration.
